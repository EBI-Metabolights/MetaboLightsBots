...
def main(arguments):
    ...
    for metabolite in metabolites:
        ...
        print metabolite + " - " + MLC_DATA['inchiKey']
        spectra = fetchSpectraFromMona(MLC_DATA['inchiKey'])
        break;


def fetchSpectraFromMona(inchikey):
    ml_spectrum = []
        ...
        for spectra in MLC_SpectraList:
            try:
                if spectra['type'] == "MS":
                    ...
                    adv_destinationPath = destination + "/" + metabolite + "/" + metabolite + "_spectra" + "/" + spectra['name'] + "/" + spectra['name'] + "_splash.json"
                    writeDataToFile(adv_destinationPath, merge_data, "w")
            except:
...
def mergeMONA_MS_SPECTRA(ml,mona):
    ...
    for datapoint in datapoints:
        ...
        intensity = float(tempArray[1].strip()) * 10
        count = 0
        #print mz + " <---> " + intensity 
